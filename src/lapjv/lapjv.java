...
public class lapjv
{	
	public static int[] execute(int[][] assigncost)
	{
...
//	  int[] u;
	  ...
	  assigncost = MatrixUtils.makeSquare(assigncost,MatrixUtils.sum(assigncost)+1);
	  int dim = assigncost.length;
	  //int dim = Math.max(assigncost.length, assigncost[0].length);
	  
	  rowsol = new int[dim];
	  colsol = new int[dim];
//	  u = new int[assigncost.length];
	  v = new int[assigncost[0].length];
